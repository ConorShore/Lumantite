  ...
  return (
    <div className="h-full flex flex-col">
      <div className="flex items-center gap-1 border-b border-slate-200 bg-slate-50 px-2 py-1 flex-wrap">
        {KINDS.map((k) => (
          <button key={k} className={`rounded px-2 py-0.5 ${k === kind ? "bg-sky-600 text-white" : "hover:bg-slate-200"}`} onClick={() => { setView(k, null); setDraftNew(null); }}>
            {k} <span className="opacity-60">{counts.get(k) ?? 0}</span>
          </button>
        ))}
        <input className="ml-auto rounded border px-1" placeholder="Search…" value={q} onChange={(e) => setQ(e.target.value)} />
        <button className="rounded border bg-white px-2" onClick={() => setDraftNew({ file: DEFAULT_FILE[kind] ?? "catalog.yaml", entry: { kind, id: `new-${kind}`, ...requiredSkeleton(kind) } as RawEntry })}>+ New</button>
        <button className="rounded bg-sky-600 px-2 text-white disabled:opacity-40" disabled={!dirty.length || saving} onClick={() => void save()} title={dirty.join(", ")}>
          {saving ? "Saving…" : `Save catalog${dirty.length ? ` (${dirty.length})` : ""}`}
        </button>
      </div>
      {error && <div className="bg-red-50 px-2 py-1 text-red-700">{error}</div>}
      <div className="flex flex-1 min-h-0">
        <div className="w-[48%] overflow-auto border-r border-slate-200">
          <table className="w-full whitespace-nowrap text-[11px]">
            <thead className="sticky top-0 bg-white shadow-sm">
              <tr className="text-left text-slate-500"><th className="px-2">id</th><th className="px-1">vendor / model</th><th className="px-1">description</th><th className="px-1">extends</th><th className="px-1">file</th><th className="text-right pr-2">used</th></tr>
            </thead>
            <tbody>
              {list.map((r) => (
                <tr
                  key={`${r.local}:${r.file}:${r.entry.id}`}
                  className={`cursor-pointer border-t border-slate-100 ${selected?.entry.id === r.entry.id && !draftNew ? "bg-sky-100" : "hover:bg-slate-50"}`}
                  onClick={() => { setDraftNew(null); setView(kind, r.entry.id); }}
                >
                  <td className="px-2 font-mono">{r.entry.id}{r.local && <span className="ml-1 rounded bg-amber-100 px-1 text-amber-800" title="project-local override (read-only here)">local</span>}</td>
                  ...
                  <td className="text-slate-500">{r.file}</td>
                  <td className="pr-2 text-right tabular-nums">{usage.get(r.entry.id) ?? 0}</td>
                </tr>
              ))}
              {!list.length && <tr><td colSpan={6} className="px-2 py-2 text-slate-400">No {kind} models.</td></tr>}
            </tbody>
          </table>
        </div>
        <div className="flex-1 min-w-0 overflow-auto">
          {selected ? (
            ...
            <div className="p-4 text-slate-400">Select a model to edit, or create a new one.</div>
          )}
        </div>
      </div>
    </div>
  ...
  return (
    <div className="p-2 space-y-2">
      <div className="flex items-center gap-2">
        <span className="rounded bg-slate-700 px-1.5 text-[10px] uppercase text-white">{entry.kind}</span>
        <input className={`rounded border px-1 font-mono ${idTaken ? "border-red-500" : ""}`} value={draft.id} onChange={(e) => setDraft({ ...draft, id: e.target.value })} disabled={readOnly} aria-label="id" />
        {entry.kind !== "wavelength-plan" && (
          <label className="flex items-center gap-1">extends
            <select className="rounded border px-1" value={(draft.extends as string) ?? ""} disabled={readOnly} onChange={(e) => { const n = { ...draft }; if (e.target.value) n.extends = e.target.value; else delete n.extends; setDraft(n); }}>
              <option value="">—</option>
              {sameKind.map((id) => <option key={id}>{id}</option>)}
            </select>
          </label>
        )}
        <span className="ml-auto text-slate-500">used by {usage} in this project</span>
      </div>
      <label className="flex items-center gap-1">file
        <input className="rounded border px-1 font-mono" list="catalog-files" value={target} disabled={!isNew || readOnly} onChange={(e) => setTarget(e.target.value)} />
        <datalist id="catalog-files">{files.map((f) => <option key={f} value={f} />)}</datalist>
      </label>
      {readOnly && <div className="text-amber-700">Project-local override: edit it in the project's catalog/ directory.</div>}
      ...
      {!readOnly && (
        <div className="flex gap-1">
          <button className="rounded bg-sky-600 px-2 text-white disabled:opacity-40" disabled={!changed || idTaken || !draft.id} onClick={apply}>{isNew ? "Add" : "Apply"}</button>
          <button className="rounded border px-2 disabled:opacity-40" disabled={!changed} onClick={() => (isNew ? onDone() : setDraft(structuredClone(entry)))}>{isNew ? "Cancel" : "Revert"}</button>
          {!isNew && entry.kind !== "wavelength-plan" && (
            <button className="rounded border px-2" title="New model that extends this one" onClick={() => onClone({ kind: entry.kind, id: `${entry.id}-copy`, extends: entry.id } as RawEntry)}>Clone (extends)</button>
          )}
          {!isNew && (
            <button className="ml-auto rounded border border-red-300 px-2 text-red-700" onClick={() => { if (!usage || window.confirm(`${entry.id} is used ${usage}× in this project. Delete anyway?`)) { remove(entry.id); onDone(); } }}>Delete</button>
          )}
        </div>
      )}
      {resolved && entry.extends !== undefined && (
        <details>
          <summary className="cursor-pointer text-slate-500">Effective model (extends resolved)</summary>
          <pre className="mt-1 max-h-72 overflow-auto rounded bg-slate-50 p-1 text-[10px]">{JSON.stringify(resolved, null, 2)}</pre>
        </details>
      )}
    </div>
  );
