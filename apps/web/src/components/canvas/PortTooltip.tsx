...
function Rows({ title, chans, total }: { title: string; chans: PortChannelResult[]; total?: { min: number; typ: number; max: number } }) {
  ...
  return (
    <>
      <tr><td colSpan={5} className="pt-1 font-semibold text-muted">{title}{total && <span className="font-normal"> — total <span className="text-aqua">{dB(total.typ)}</span> dBm</span>}</td></tr>
      {chans.slice(0, 20).map((c) => (
        <tr key={c.signalId} className="tabular-nums">
          ...
          <td className="pr-1 text-right font-medium text-aqua">{dB(c.power.typ)}</td>
          <td className="pr-2 text-right">{dB(c.power.max)}</td>
          <td className="text-right">{cd(c.cd)}</td>
        </tr>
      ))}
      {chans.length > 20 && <tr><td colSpan={5} className="text-muted">… {chans.length - 20} more</td></tr>}
    </>
  );
}
...
export function PortTooltip() {
  ...
  return (
    <div className="fixed z-50 pointer-events-none rounded border border-line bg-raised px-2 py-1 text-fg shadow-xl shadow-black/40 text-[10px]" style={{ left: hover.x + 12, top: hover.y + 12 }} role="tooltip">
      <div className="font-semibold font-mono text-aqua">{hover.node}.{hover.port}</div>
      {!pr || (!pr.in.channels.length && !pr.out.channels.length) ? (
        <div className="text-muted">no signal</div>
      ) : (
        <table>
          <thead><tr className="text-muted"><th className="text-left pr-2">ch</th><th className="pr-1">min</th><th className="pr-1">typ</th><th className="pr-2">max dBm</th><th>CD ps/nm</th></tr></thead>
          <tbody>
